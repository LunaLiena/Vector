...
export const TaskStatusService = {
  ...
  swr: {
    useTaskStatuses: () =>
      createSWRService('task-statuses', TaskStatusService.getTaskStatuses).useQuery(),
    useTaskStatusById: (id: number) =>
      createSWRService('task-status-by-id', TaskStatusService.getTaskStatusById).useQuery(id),
    mutateAll: () => {
