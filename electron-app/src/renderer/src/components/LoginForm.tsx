    ...
    <MotionDiv>
      <Card view="raised" className="p-6" style={{ padding: '32px' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Container>
            <CustomLabel className="mb-1">Пароль</CustomLabel>
            <TextInput
              id="password"
              type="password"
...
const CustomLabel = ({ children, htmlFor, className = '' }: CustomLabelProps) => {
  return (
    <label
      htmlFor={htmlFor}
      className={`block text-sm font-medium text-gray-700 mb-2 tracking-wide ${className}`}
      style={{ paddingTop: 12 }}
    >
      {children}
    </label>
...
const Container = ({ children }: ContainerProps) => <div style={{ marginTop: 12 }}>{children}</div>;
